...
async def batch_init_users(n: int, n_items: int, n_users: int, item_price: int):
    try:
        await batch_init_users_db(n, n_items, n_users, item_price)
...
async def send_post_request(url: str, body: dict = {}):
    try:
        if body:
...
async def checkout(order_id: str):
    ...
    stock_reply = await send_post_request(f"{GATEWAY_URL}/stock/subtract_bulk/{order_id}", body=items_quantities)
    if stock_reply.status_code == 200:
        payment_reply = await send_post_request(f"{GATEWAY_URL}/payment/pay_order/{order_id}")
        if payment_reply.status_code == 200:
            app.logger.debug("Checkout successful")
            return Response("Checkout successful", status=200)
        else:
            if payment_reply.status_code == 405:
                abort(400, "User out of credit")
            else:
                return abort(400, DB_ERROR_STR)
    else:
        if stock_reply.status_code == 405:
            abort(400, f'Out of stock on item_id: {item_id}')
        else:
            return abort(400, DB_ERROR_STR)
